...
from my_input import extract_data, resize_with_pad, IMAGE_SIZE

import theano
#theano.config.blas.ldflags = "-LC:\Users\Eric\OpenBLAS\bin-lopenblas"
...
class Dataset(object):
    ...
    def read(self, img_rows=IMAGE_SIZE, img_cols=IMAGE_SIZE, img_channels=3, nb_classes=2):
        images, labels = extract_data('./training')
        print('images shape:',images.shape)
        print('labels shape:',labels.shape)
	print('lables[0]:', labels[0])
	print('lables[1]:', labels[1])
	print('lables[2]:', labels[2])
	print('lables[3]:', labels[3])
        #labels = np.reshape(labels, [-1])
        # 训练集、测试集划分
        # allow_nd = True表示允许输入超过2维的数据
        ...
        # 更改数据集的形状结构
        if K.image_dim_ordering() == 'th': # This for Theano
            X_train = X_train.reshape(X_train.shape[0], 3, img_rows, img_cols)
            X_valid = X_valid.reshape(X_valid.shape[0], 3, img_rows, img_cols)
            X_test = X_test.reshape(X_test.shape[0], 3, img_rows, img_cols)
            input_shape = (3, img_rows, img_cols)
        else: #this for tensorflow
...
if __name__ == '__main__':
    ...
    model.train(dataset, nb_epoch=1)
    model.save()
